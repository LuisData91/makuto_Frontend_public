...
import { listarProductos } from '../../productos/servicio/productoService';

const FormBProducto = ({ show, onClose, onSelect, closeOnSelect = false }) => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const { data, pagination: pg } = await listarProductos({ q, page, per_page: perPage });
            setRows(data || []);
            ...
            console.error(err);
            setRows([]); setPaginaActual(1); setTotalPaginas(1); setTotalRegistros(0);
        } finally { setLoading(false); }
    };

    useEffect(() => { if (canShow) fetchData(); }, [canShow, page, perPage, q]);

    const handleSelect = (prod) => {
        onSelect?.(prod);
        if (closeOnSelect) onClose?.(); // ← cierra solo si se indicó
        // si no, queda abierto para agregar más
    };
                ...
                <div className="modal-dialog modal-xl modal-dialog-scrollable">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                                ...
                                <table className="table table-hover align-middle table-sm">
                                    ...
                                    <tbody>
                                        {loading ? (
                                            ...
                                            <tr>
                                                <td colSpan={3} className="text-muted">Sin resultados</td>
                                            </tr>
                                        ) : (
                                            rows.map((r) => (
                                                <tr key={r.prod_id}>
                            ...
                            <div className="d-flex justify-content-between align-items-center">
                                ...
                                <div className="d-flex align-items-center gap-2">
                                    ...
                                    <span className="mx-2">{paginaActual} / {totalPaginas}</span>
                                    <button
                                        type="button"
                                        className="btn btn-outline-secondary btn-sm"
                        ...
                        <div className="modal-footer">
                            {/* Si seleccionas varios y luego cierras manual, perfecto */}
